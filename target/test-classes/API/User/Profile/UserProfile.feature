Feature: As a user, I want to be able to update the user profile information in the system through API connection.
  ...
  response body should be verified as "Profile updated successfully"
    ...
    #Api kullanicisi patch request gonderir ve user profile endpointinden donen responsei gecerli authorization bilgisi ile kaydeder
  ...
  code is 401, and the error message in the response body is "Unauthorized request"
    ...
    #Api kullanicisi user profile endpointinden donen responsei geçersiz authorization bilgisi ile kaydeder, status codeun 401 ve error bilgisinin Unauthorized oldugunu dogrular

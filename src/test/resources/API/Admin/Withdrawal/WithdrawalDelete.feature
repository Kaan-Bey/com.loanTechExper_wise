Feature: As an administrator, I should be able to delete a withdrawal record in the system through the API connection.
  ...
  "withdrawal deleted"
    ...
    #Apı kulanıcısı api withdrawal delete endpointinden donen responseı geçerli authorization bilgisi ile kaydeder
  ...
  is 401. Furthermore, the error information in the response body should be confirmed as "Unauthorized request"
    ...
    #Api kullanicisi api withdrawal delete endpointinden donen responsei geçersiz authorization bilgisi ile kaydeder, status codeun 401 ve error bilgisinin Unauthorized oldugunu dogrular
